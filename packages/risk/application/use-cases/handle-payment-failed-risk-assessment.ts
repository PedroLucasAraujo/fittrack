...
import type { HandlePaymentFailedRiskAssessmentInputDTO } from '../dtos/handle-payment-failed-risk-assessment-input-dto.js';

/**
 * Automated risk assessment triggered by payment failure events (ADR-0053 §1).
 *
 * ## Escalation logic
 *
 * | Current RiskStatus | paymentFailureCount >= limit | Outcome |
 * |--------------------|------------------------------|---------|
 ...
 *
 * ## Threshold
 * Default: ≥ 3 payment failures in 30 days → WATCHLIST (ADR-0053 §1, Invariant 1).
 * Thresholds are read from `RiskThreshold.defaults()` — no magic numbers (ADR-0053 §6).
 *
 ...
 */
export class HandlePaymentFailedRiskAssessment {
  constructor(
  ...
  async execute(dto: HandlePaymentFailedRiskAssessmentInputDTO): Promise<DomainResult<void>> {
    // 1. Validate and construct RiskIndicators VO
    const indicatorsResult = RiskIndicators.create({
      paymentFailureCount: dto.paymentFailureCount,
      cancellationRate: 0,
    ...
    if (!indicators.isPaymentFailureThresholdExceeded(threshold)) {
    ...
    const reason = `Payment failure threshold exceeded: ${indicators.paymentFailureCount} failures in ${indicators.windowDays}d (limit: ${threshold.paymentFailureLimit})`;
