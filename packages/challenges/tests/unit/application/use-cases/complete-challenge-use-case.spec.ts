...
describe('CompleteChallengeUseCase', () => {
  ...
  it('determines winners correctly for WINNER policy (top 1)', async () => {
    ...
    expect(result.value.winners[0]!.userId).toBe(user2); // highest progress
    expect(result.value.winners[0]!.rank).toBe(1);
  });
  ...
  it('determines winners correctly for TOP_3 policy', async () => {
    ...
    expect(result.isRight()).toBe(true);
    expect(result.value.winners).toHaveLength(3);
    expect(result.value.winners[0]!.rank).toBe(1);
    expect(result.value.winners[1]!.rank).toBe(2);
    expect(result.value.winners[2]!.rank).toBe(3);
  });
  ...
  it('determines winners correctly for TOP_10 policy', async () => {
    ...
    expect(result.isRight()).toBe(true);
    expect(result.value.winners).toHaveLength(10);
    expect(result.value.winners[0]!.progress).toBe(15);
    expect(result.value.winners[9]!.progress).toBe(6);
  });
  ...
  it('determines winners correctly for ALL_COMPLETERS policy', async () => {
    ...
    const winnerIds = result.value.winners.map((w) => w.userId);
  ...
  it('returns empty winners array when no participants', async () => {
    ...
    expect(result.isRight()).toBe(true);
    expect(result.value.winners).toHaveLength(0);
  ...
  it('sorts participants by completedAtUtc ASC when progress is tied', async () => {
    ...
    expect(result.isRight()).toBe(true);
    expect(result.value.winners[0]!.userId).toBe(earlyUser); // earlier completer wins tie
  });
});
